...
if __name__ == "__main__":
    ...
    import gzip, struct
    # data_name='20200528.000000'
    # picture_name='2020052800'

    # data_name='20200826.040000'
    picture_name='20200526?04'
    
    # data_name='20200522.040000'
    # picture_name='2020052204'

    data_name='20200529.060000'
    picture_name='2020052906'
    save_path = 'Result/ConvLSTM_nokmean_radarshow_alltaiwa_641X641_512/CREF_{}pred6/'.format(data_name)
    
    if not os.path.isdir(save_path):
        os.makedirs(save_path)
    
#    gz = gzip.open("NWP/COMPREF_OBS_FCST/COMPREF_OBS_FCST/CREF_010min/CREF_010min.{}.gz".format(data_name)).read()
    gz = gzip.open("CREF/CREF_010min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}10'.format(picture_name), savepath=save_path)
    
    gz = gzip.open("CREF/CREF_020min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}20'.format(picture_name), savepath=save_path)
    
    gz = gzip.open("CREF/CREF_030min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}30'.format(picture_name), savepath=save_path)
    
    gz = gzip.open("CREF/CREF_040min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}40'.format(picture_name), savepath=save_path)
    
    gz = gzip.open("CREF/CREF_050min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}50'.format(picture_name), savepath=save_path)
    
    gz = gzip.open("CREF/CREF_060min.{}.gz".format(data_name)).read()
    radar = np.array(struct.unpack(1501*1501*'h', gz[-1501*1501*2:])).reshape(1501, 1501).astype(np.float64)/10
    print(radar)
    print(np.array(radar).shape)
    visualized_area_with_map2(radar, 'Sun_Moon_Lake', [641, 641], title='{}100'.format(picture_name), savepath=save_path)
